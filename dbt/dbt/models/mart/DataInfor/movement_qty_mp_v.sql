...
select
  ...
  0 AS Adjust4,
  0 AS Adjust5
from {{ source('mp_infor', 'saldo_awal_qty_mp_bq') }} mq_mp
UNION ALL
select
  mat.Item,
  mat.ItemDescription,
  mat.ProductCodeDescription AS ue_ProdDesc,
  c.ue_CustName,
  c.ue_ConvFactor,
  c.ue_UM,
...
from {{ ref('material_transaction_v') }} mat
LEFT JOIN (select 
              tt.RefType,
              tt.TransNum
            from {{ source('mp_infor', 'transtype_MP') }} tt
...
LEFT JOIN (select 
              ...
              saldo_pr.ue_UM
          from {{ source('mp_infor', 'saldo_awal_qty_mp_bq') }} saldo_pr
...
where site = 'MP'
GROUP BY mat.Item, mat.ItemDescription, mat.ProductCodeDescription, mat.ItmUM,site, month_year,  c.ue_CustName,
  c.ue_ConvFactor,
  c.ue_UM

) x
-- where x.ue_Item='100100001' and x.month_year='2023-07'
 group by x.ue_Item, 
  x.ue_ItemDesc,
  x.ue_ProdDesc,
  x.ue_CustName,
