...
SELECT 
  ...
  sales_tran.CgsMatlTotal,
  sales_tran.CgsLbrTotal,
  sales_tran.CgsFovhdTotal,
  sales_tran.CgsVovhdTotal,
  sales_tran.CgsOutTotal,
  sales_tran.CgsTotal
FROM {{ source('mp_infor', 'shipment_or_fi_detail') }} a
  ...
  LEFT JOIN (select 
                ...
                CgsOutTotal,
                CgsTotal
              from {{ source('mp_infor', 'MP34_SalesTransaction') }}) sales_tran ON a.ue_inv_num = sales_tran.InvNum
                                                    AND a.ue_co_num = sales_tran.CoNum
                                                    AND a.ue_co_line = CAST(sales_tran.CoLine AS INT64)
-- where a.ue_inv_num='HBI/2024-061' AND a.ue_co_num='CO-0001401'
