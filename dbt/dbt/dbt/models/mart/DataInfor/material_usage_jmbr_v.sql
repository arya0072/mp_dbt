...
LEFT JOIN (
  ...
  where ue_Qty <> 0 and ue_whse IN ('PRJM','Jembrana')
...
Where ue_whse IN ('PRJM','Jembrana')
UNION ALL
SELECT 
  'Absorbed' AS type,
  actual.type AS type_actual,
  ...
  'JEMBRANA' AS ue_whse,
  absorbed.ue_TotalLaborCost
FROM {{ source('mp_infor', 'material_usage_jmbr') }} AS absorbed
LEFT JOIN (
  SELECT 
    'Actual' AS type,
    ue_TransDate,
    ...
    ue_Qty,
    ue_TotalMaterialCost
  FROM {{ source('mp_infor', 'cost_of_production_jmbr') }}
) actual ON absorbed.ue_item = actual.ue_item AND absorbed.ue_Job = actual.ue_job
LEFT JOIN (
  SELECT
    itmUf_MP123_StandartCost,
    Item
  FROM {{ source('mp_infor', 'standard_cost_jmbr') }} 
) standard ON absorbed.ue_item = standard.Item
