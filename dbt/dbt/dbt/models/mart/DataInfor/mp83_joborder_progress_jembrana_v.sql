...
FROM {{ source('mp_infor', 'mp83_joborder_progress_jemb') }} AS mp83_job_progress 
  LEFT JOIN {{ source('mp_infor', 'uom_conversion') }} AS uom_conv ON mp83_job_progress.Item = uom_conv.Item AND uom_conv.ToUM NOT IN ('UNT')
-- where mp83_job_progress.Item like '%5006%'