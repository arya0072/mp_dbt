...
FROM {{ source('mp_infor', 'hris_user') }} user
  JOIN {{ source('mp_infor', 'mp80_incentives') }} a ON TRIM(a.EmpNum) = user.nik                                    
...
WHERE (a.Gross > 0 AND a.TargetQty > 0) 
  AND a.IncentiveDate >= '2025-01-01'
  -- AND TRIM(a.EmpNum)='254186'
  AND SUBSTR(a.Job, 1, 5) IN ('JSFG-','JSFJ-','JSMJ-','JSFB-')  -- JO Gianyar & Jembrana
  AND user.JobTitle IN ('Control Girl','Rolling Girl Cones','Glue Girl','Rolling Girl Filter')
  AND user.SecondApprove NOT IN ('Ni Nyoman Triani')
