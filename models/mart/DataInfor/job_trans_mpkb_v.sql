...
SELECT 
  ...
  CASE
        ...
        WHEN SUBSTR(min_wc.JobrWc, 1,2) IN ('M-') THEN 'Mesin-Rolling'
        ELSE 'NotIdentify'
    END as JobType,
    a.TransDate,
    a.JobItem,
...
FROM {{ source('mp_infor', 'job_transaction_mp') }} a
  ...
  GROUP BY 
    ...
    AND job_trans.Whse LIKE '%KB%' -- hanya lokasi MPKB
---------------------------------------------------------------------------------------------------------------------------------------
UNION ALL
---------------------------------------------------------------------------------------------------------------------------------------
SELECT
...
FROM {{ source('mp_infor', 'job_transaction_mpkb') }} a
  JOIN (select 
          UPPER(Job) as Job,
          MAX(OperNum) as opernum        
          from {{ source('mp_infor', 'job_transaction_mpkb') }}
          -- where Job='JFG-016328' 
  ...
  JOIN (
        SELECT 
          ...
          SUM(xx.QtyMoved) as QtyMoved,
          MAX(ue_CreateDate) as ue_CreateDate
        FROM {{ source('mp_infor', 'job_transaction_mpkb') }} xx
          JOIN (SELECT
                  UPPER(Job) as Job,
                  MAX(OperNum) as OperNum
                FROM {{ source('mp_infor', 'job_transaction_mpkb') }} 
