...
(select
    ...
    Item.description,
    curr_mat.item as item_material,
    curr_mat.ItmDescription as desc_item_material,
    CASE
...
FROM {{ source('mp_infor', 'items') }} item
  LEFT JOIN {{ source('mp_infor', 'current_material') }} curr_mat ON item.Job =  curr_mat.Job 
                                             AND item.Suffix = curr_mat.Suffix
  LEFT JOIN {{ source('mp_infor', 'item_attribute_all') }} item_att ON item_att.RefRowPointer = item.RowPointer
  LEFT JOIN (select
                a.AttributeValue,
                a.RefRowPointer
...
where b.Item LIKE '9005%' OR b.Item LIKE '9006%'
